...
class Format:
    def __init__(self, **kwargs):
        ...
        # Cache valid methods for faster subsequent instantiations (class attribute)
        cls = self.__class__
        if not hasattr(cls, "_valid_methods"):
            cls._valid_methods = [
                m for m in dir(cls) if m[0] != "_" and m != "to_plotly_json"
            ]
        valid_methods = cls._valid_methods
    ...
    def _validate_string(self, value):
        # Highly optimized type check for string (str)
    ...
    def symbol_suffix(self, value):
        # Avoid function call overhead for _validate_string, inline fast type check
        if type(value) is not str:
            raise TypeError("expected value to be a string")

        locale = self._locale
        if "symbol" in locale:
            locale["symbol"][1] = value
        else:
            locale["symbol"] = ["", value]
        return self
