...
class Format:
    ...
    def _validate_string(self, value):
        # Faster isinstance check (removes "".__class__)
    ...
    def symbol_prefix(self, value):
        # Inline type check for micro-optimization
        if not isinstance(value, str):
            raise TypeError("expected value to be a string")
        # Use local variable for faster dict access
        locale = self._locale
        locale.setdefault("symbol", [value, ""])
        locale["symbol"][0] = value
        return self
