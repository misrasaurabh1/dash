...
class Format:
    def __init__(self, **kwargs):
        ...
        # Prepare valid_methods as a set for fast lookup
        valid_methods = set(
        ...
        for kw, val in kwargs.items():
            ...
            # getattr and call remain unchanged for dynamic method invocation
            getattr(self, kw)(val)

    def _validate_char(self, value):
        # Inline the string check to save function call overhead
    ...
    def fill(self, value):
        # Inline and localize hot accesses to avoid attribute lookup overhead
        if type(value) is not str:
            raise TypeError("expected value to be a string")
        if len(value) != 1:
            raise ValueError("expected value to a string of length one")
