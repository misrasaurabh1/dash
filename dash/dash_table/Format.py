...
class Format:
    ...
    def _validate_non_negative_integer_or_none(self, value):
        # Fast-path: most frequent check first (None or integer >= 0)
        if value is None or (type(value) is int and value >= 0):
            return
        # Only check for not-int, negative, or exotic objects if above didn't pass
    ...
    def padding_width(self, value):
        # Avoid call overhead for the likely cases; do quick inlined check first
        if value is None or (type(value) is int and value >= 0):
