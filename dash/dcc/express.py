...
def send_data_frame(writer, filename, type=None, **kwargs):
    ...
    # Check if the provided writer is known, avoid .keys() for faster lookup.
    if name not in _data_frame_senders:
        raise ValueError(
            f"The provided writer ({name}) is not supported, "
            "try calling send_string or send_bytes directly."
        )
